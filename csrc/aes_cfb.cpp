    ...
    jint outputOffset)
{
  try {
    ...
    // init
    {
      ...
      aes_cfb_cipher.init(opMode, padding, j_key.get(), keyLen, j_iv.get());
    }
...

    {
      ...
      result = aes_cfb_cipher.update(input.get() + inputOffset, inputLen, output.get() + outputOffset, 0);
    }

    // final
    result += aes_cfb_cipher.do_final(output.get() + outputOffset + result);
    ...
    jint outputOffset)
{
  try {
    ...
    return aes_cfb_cipher.update(input.get() + inputOffset, inputLen, output.get() + outputOffset, 0);
